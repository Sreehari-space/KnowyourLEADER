...
export default function AnimatedCandidateModal({ candidate: rawCandidate, lang, fontSize, onClose }: CandidateModalProps) {
  ...
  useEffect(() => {
    ...
    loadCandidateDetails(rawCandidate.id).then(details => {
      if (!cancelled) { setCandidate(mergeDetails(rawCandidate, details)); setDetailsLoading(false); }
    }).catch(() => {
      if (!cancelled) setDetailsLoading(false);
      /* Discrepancies simply stay hidden if the chunk fails to load. */
    });
  ...
  const getEasyReadText = () => {
    ...
    let text = rawTemplate
      ...
      .replace('{networth}', FORMAT_NET_WORTH(candidate));
  ...
  /**
   * Affidavit vehicle strings are one run-on line of repeating records:
   *   "Car BMW 530 TN 07 CV 3031 year 2020 80,54,119 80 Lacs+ Car Toyoto ..."
   * Each record ends with the rounded magnitude marker, which is a reliable
   * separator. An earlier version stripped every number first, which orphaned
   * the bare word "year" into its own entry and dropped model designations,
   * producing "Car BMW • year • Car Toyoto Lexus • year • …".
   */
  const formatVehicleData = (rawText?: string) => {
    const none = lang === 'en' ? 'None Declared' : 'ஏதுமில்லை';
    if (!rawText || rawText.toLowerCase() === 'nil' || rawText === 'Not Applicable') return none;

    const STATE = 'TN|KL|KA|AP|TS|MH|DL|PY|GJ|RJ|UP|MP|WB|HR|PB|OD|BR|JH|CG|AS|GA|HP|JK|UK|CH';
    const plate = new RegExp(`\\b(?:${STATE})\\s*-?\\s*\\d{1,2}\\s*-?\\s*[A-Z?]{0,3}\\s*-?\\s*(?:[A-Z]{1,2}\\s*-?\\s*)?\\d{1,4}\\b`, 'gi');

    const vehicles = rawText
      .split(/(?:Thou|Lacs?|Crores?|Lakhs?)\s*\+/i)
      .map((record) => {
        let s = ` ${record} `;
        s = s.replace(plate, ' ');
        const ym =
          s.match(/\b(?:year|yr)\.?\s*((?:19|20)\d{2})\b/i) ||
          s.match(/\(\s*((?:19|20)\d{2})\s*\)/) ||
          s.match(/\b((?:19|20)\d{2})\s*,/) ||
          s.match(/\b((?:19|20)\d{2})\b(?=[^\d]*$)/);
        const year = ym ? ym[1] : '';
        s = s.replace(/\b(?:year|yr)\.?\s*(?:(?:19|20)\d{2})?\b/gi, ' ');
        if (year) s = s.replace(new RegExp(`\\(?\\s*\\b${year}\\b\\s*\\)?,?`), ' ');
        // The value is the trailing number group or two (exact + magnitude).
        s = s.replace(/(?:\s+[\d,]+(?:\.\d+)?){1,2}\s*$/, ' ');
        s = s.replace(/\*?\(?\s*Value Not Given\s*\)?/gi, ' ');
        s = s.replace(/\b(?:Rs|Value|Purchase|Date|Model|Make|Regn|Registration|No)\b\.?/gi, ' ');
        s = s.replace(/[-*:;+?,]/g, ' ');
        const name = s.replace(/\s+/g, ' ').trim();
        return name.length > 2 ? { name, year } : null;
      })
      .filter((v): v is { name: string; year: string } => v !== null);

    if (!vehicles.length) return none;

    return (
      <span className="flex flex-col gap-1.5 mt-1">
        {vehicles.map((v, i) => (
          <span key={i} className="flex items-baseline gap-2 leading-snug">
            <span className="text-[10px] font-mono font-bold text-slate-400 shrink-0 tabular-nums">{i + 1}.</span>
            <span className="flex-1 min-w-0">{v.name}</span>
            {v.year && (
              <span className="shrink-0 text-[10px] font-mono font-bold text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded">
                {v.year}
              </span>
            )}
          </span>
  ...
  const formatLandData = (rawText?: string) => {
    ...
    const parsed = entries.map((entry) => {
      ...
      const areaMatch = entry.match(/Total Area\s+([^\s]+\s+[a-zA-Z.]+)/i);
    ...
    if (!parsed.length) return rawText;
              ...
              <div className="max-w-3xl mx-auto space-y-6 md:space-y-8">
                ...
                <div className="bg-indigo-600 rounded-3xl p-6 md:p-8 shadow-xl shadow-indigo-600/20 text-white relative overflow-hidden flex flex-col md:flex-row items-start md:items-center justify-between">
                  ...
                  <div className="relative z-10">
                    ...
                    <p className="text-4xl md:text-6xl font-black font-mono tracking-tighter mt-2">{FORMAT_NET_WORTH(candidate)}</p>
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-stretch">
                  <MovableAssetCard 
                    ...
                    className="max-h-[320px] md:h-[350px]"
                  />
                  {/* Immovable Assets (Land & Properties) */}
                  <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col max-h-[320px] md:h-[350px]">
                  ...
                  <div className="md:col-span-2">
                    <MovableAssetCard 
                      ...
                      lang={lang} 
                      className="max-h-[280px] md:h-[300px]"
                    />
                  </div>
                </div>
